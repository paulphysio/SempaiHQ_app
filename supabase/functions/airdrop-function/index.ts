...
import { ComputeBudgetProgram, Connection, Keypair, PublicKey, Transaction, sendAndConfirmTransaction } from "npm:@solana/web3.js@1";
...
function generateKey(secret) {
  console.log("[crypto] Generating key from secret");
...
// Helper to Create or Get User Wallet
async function getOrCreateUserWallet(user_id) {
  console.log("[wallet] Processing for user_id:", user_id);
  const newKeypair = Keypair.generate();
  const encryptedPrivateKey = encrypt(newKeypair.secretKey, KEYPAIR_ENCRYPTION_SECRET);
  ...
  const userWallet = await supabaseAdmin.from("user_wallets").select().eq("user_id", user_id).limit(1).then(throwOnError);
  if (!userWallet || userWallet.length === 0) {
    console.error("[wallet] No wallet returned for user_id:", user_id);
  ...
  await supabaseAdmin.from("wallet_balances").upsert({
    ...
    amount: 0,
    decimals: 6,
    wallet_address: userWallet[0].address
  }).then(throwOnError);
  console.log("[wallet] Wallet and balance initialized for address:", userWallet[0].address);
  return userWallet[0];
}
// Main Handler
Deno.serve(async (req)=>{
  ...
  // Route: Create Wallet
  if (route === "create-wallet") {
    try {
      const userWallet = await getOrCreateUserWallet(user_id);
      console.log("[wallet] Wallet created successfully for user_id:", user_id);
      return new Response(JSON.stringify({
      ...
      console.error("[wallet] Error:", e.message, e.stack);
      return new Response(JSON.stringify({
  ...
  if (route === "airdrop") {
    ...
    // Check Total Claims
    try {
      const { count } = await supabaseAdmin.from("airdrop_transactions").select("id", {
        count: "exact"
      });
      ...
      console.error("[airdrop] Airdrop limit check error:", e.message, e.stack);
      return new Response(JSON.stringify({
        error: "Failed to check airdrop limit: " + e.message
      }), {
        status: 500,
        headers: {
          "Content-Type": "application/json"
    ...
    try {
      const { data: userActivity, error } = await supabaseAdmin.from("user_activity").select("has_claimed_airdrop").eq("user_id", user_id).limit(1);
      if (error) {
        console.error("[airdrop] Eligibility query error:", error.message, error.stack);
        throw error;
      }
    ...
    try {
      ...
      }), createAssociatedTokenAccountInstruction(AIRDROP_KEYPAIR.publicKey, smpAta.user, user, SMP_MINT_ADDRESS), createTransferInstruction(smpAta.treasury, smpAta.user, AIRDROP_KEYPAIR.publicKey, AIRDROP_SMP_AMOUNT));
