      ...
      for (let i = 0; i < chapters.length; i++) {
        ...
        if (existing) {
          const { error } = await supabase
            .from('manga_chapters')
            .update({ title: chapter.title, is_premium: chapter.isPremium })
            .eq('id', existing.id);
          ...
          const { data, error } = await supabase
            .from('manga_chapters')
            .insert([{ manga_id: mangaId, chapter_number: i + 1, title: chapter.title, is_premium: chapter.isPremium }])
            .select('id')
            .single();
    ...
    ({ item, index }) => (
      <Animated.View entering={FadeIn} exiting={FadeOut} style={styles.chapterItem}>
        <Text style={styles.chapterTitleText}>
          {item.title} ({item.pages.length} pages) {item.isPremium && '[Premium]'}
        </Text>
